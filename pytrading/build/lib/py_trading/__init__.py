...
class Stock:
	...
	def _price_predictions(self.ticker):
	...
	def _news_sentiments(self.ticker): # Returns news articles curated via Finviz, Yahoo, and Google News, GET UNUSUAL OPTION ACTIVITY
		BASE_URL = f'https://finviz.com/quote.ashx?t={ticker}'
		soup = _get_soup(BASE_URL)
	...
	def _financials(self.ticker): # OMEGALUL
	...
	def _short_selling(self.ticker):
		BASE_URL = f'https://finviz.com/quote.ashx?t={ticker}'
		soup = _get_soup(BASE_URL)
	...
	def _put_call_ratio(self.ticker): 
	...
	def _find_competition(self.ticker):
		BASE_URL = f'https://finviz.com/quote.ashx?t={ticker}'
		soup = _get_soup(BASE_URL)
	...
	def _insider_trading(self.ticker):
		BASE_URL = f'https://finviz.com/quote.ashx?t={ticker}'
		soup = _get_soup(BASE_URL)
	...
	def _catalysts(self.ticker): # Returns date of showcases, FDA approvals, earnings, etc
		# Earnings date: 
	...
	def _big_money(self.ticker): # Returns recent institutional investments in a stock, as well as the largest shareholders and mutual funds holding the stock
